...
const ConversionFormDataSchema = z.object({
  ...
  "max-notes-at-once": z.coerce.number().nonnegative().gte(1).lte(6), // standard guitars cannot play more than 6 notes at once
...
export async function convertMidiToTab(formData: FormData): Promise<State> {
  ...
  const slices = slicer({
    ...
    maxConcurrentNotes: data["max-notes-at-once"],
  });
